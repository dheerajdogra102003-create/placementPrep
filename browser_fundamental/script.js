// Browser Fundamentals Application Logic
(function () {
    'use strict';

    const MODULE_ID = 'browser_fundamental';

    // Application State
    let allQuestions = [];       // Master list of questions
    ...
    // Exam Timer State
    const EXAM_DURATION_SECONDS = 20 * 60; // 20 minutes for 30 questions
    ...
    let currentCatFilter = 'all';  // 'all' | 'theoretical_conceptual' | 'technical_application' | 'scenario_based' | 'problem_solving'
    let currentDiffFilter = 'all'; // 'all' | 'Easy' | 'Moderate' | 'Hard'
    ...
    const badgeQCategory = document.getElementById('badge-q-category');
    ...
    // Helper: normalize difficulty label
    ...
    // Theme Setup (shares storage key with cloud_Computing and dashboard)
    function initTheme() {
    ...
    function loadQuestions() {
        if (typeof questionsData !== 'undefined' && Array.isArray(questionsData) && questionsData.length > 0) {
            ...
            console.log(`Loaded ${allQuestions.length} Browser Fundamentals questions from questions.js`);
        } else {
    ...
    function bindEvents() {
        ...
        // Category filter pills in Practice Mode
        document.querySelectorAll('[data-filter-cat]').forEach(pill => {
            pill.addEventListener('click', (e) => {
                document.querySelectorAll('[data-filter-cat]').forEach(p => p.classList.remove('active'));
                e.currentTarget.classList.add('active');
                currentCatFilter = e.currentTarget.dataset.filterCat;
                applyFilters();
            });
        });

        // Difficulty filter pills in Practice Mode
    ...
    function applyFilters() {
        ...
        if (currentDiffFilter !== 'all') {
            filtered = filtered.filter(q => normalizeDiff(q.difficulty).toLowerCase() === currentDiffFilter.toLowerCase());
        }
    ...
    function startExamTimer() {
        ...
        timerInterval = setInterval(() => {
            ...
            if (examTimeRemaining <= 180) { // 3 minutes warning
    ...
    function renderCurrentQuestion() {
        ...
        badgeQCategory.textContent = formatCategory(q.category);
        
        const normDiff = normalizeDiff(q.difficulty);
        badgeQDiff.textContent = normDiff;
        badgeQDiff.className = `badge badge-diff ${normDiff.toLowerCase()}`;
        badgeQTopic.textContent = q.topic || 'Browser Fundamentals';
    ...
    function handleOptionClick(q, selectedLetter) {
        ...
        if (window.SyncManager && currentMode !== 'exam') {
            ...
            window.SyncManager.recordAnswer(MODULE_ID, qId, { selected: selectedLetter, isCorrect: isCorrect });
        }
    ...
    function showExplanationPanel(q) {
        ...
        if (userChoice !== undefined) {
            if (isCorrect) {
                ...
                explanationStatusSub.textContent = `Great job! Option ${q.correctAnswer} is correct.`;
            } else {
                explanationStatusIcon.textContent = '✗';
                ...
                explanationStatusSub.textContent = `The correct answer is Option ${q.correctAnswer}.`;
            }
        } else {
        ...
        explanationRealWorld.textContent = q.realWorldApplication || 'Understanding this core concept helps in building performant and secure web applications.';
    ...
    function showResultsScreen() {
        ...
        Object.keys(categoryStats).forEach(cat => {
            ...
            card.innerHTML = `
                <div class="breakdown-card-header">
                    <span class="breakdown-diff-dot blue"></span>
                    <div>
        ...
        if (window.SyncManager) {
            window.SyncManager.recordExamResult(MODULE_ID, {
                ...
                accuracy: percent,
                timeTaken: resultsTimeTaken.textContent
            });
        }
    }
